<div class="profile mt-3" role="region" aria-labelledby="profile-heading">
    <div class="form-wrapper mx-auto">
        <div class="form-wrapper mx-auto">
            <ol class="d-flex align-items-center text-center gap-4 mb-4 ticket-tips" aria-label="Ticket Earning Tips">
                <li><span>🎟️ Login every day to <strong>receive a ticket</strong>.</span></li>
                <li><span>📝 Complete your profile to <strong>earn 10 tickets</strong>.</span></li>
                <li><span>🔗 Connect a social account to <strong>earn 10 tickets</strong>.</span></li>
            </ol>
            ...
            <form wire:submit.prevent="save" aria-describedby="profile-description">
                ...
                <div class="row mb-4">
                    <div class="col-12">
                        ...
                        <span id="profile-image-desc" class="visually-hidden">Choose a picture to represent your
                            profile</span>

                        <!-- Label as custom trigger -->
                        <label for="profile-image"
                            class="custom-file-label d-flex align-items-center justify-content-between mt-2"
                            ...
                            @if ($profile_picture)
                                <img src="{{ $profile_picture->temporaryUrl() }}" alt="New profile picture preview"
                                    width="100">
                            @elseif ($existing_profile_picture)
                                <img src="{{ Storage::url($existing_profile_picture) }}" alt="Current profile picture"
                                    width="100">
                            @endif
                        </label>

                        <!-- Actual hidden input -->
                        <input type="file" id="profile-image" wire:model="profile_picture" name="profile_picture"
        ...
        <div class="social-login" role="group" aria-label="Connect social accounts">
            <button wire:click="redirectToGoogleLogin" type="button"
                ...
                {{ in_array('google', $connected_providers) ? 'disabled' : '' }} aria-label="Connect Google account"
                aria-disabled="{{ in_array('google', $connected_providers) ? 'true' : 'false' }}">
            ...
            <button wire:click="redirectToTwitterLogin" type="button"
                ...
                {{ in_array('twitter', $connected_providers) ? 'disabled' : '' }} aria-label="Connect Twitter account"
                aria-disabled="{{ in_array('twitter', $connected_providers) ? 'true' : 'false' }}">
            ...
            <button wire:click="redirectToDiscordLogin" type="button"
                ...
                {{ in_array('discord', $connected_providers) ? 'disabled' : '' }} aria-label="Connect Discord account"
                aria-disabled="{{ in_array('discord', $connected_providers) ? 'true' : 'false' }}">
        ...
        <div class="offcanvas offcanvas-end" tabindex="-1" id="notificationCanvas"
            ...
            <div class="offcanvas-header">
                ...
                <button type="button" class="btn-close" data-bs-dismiss="offcanvas"
                    aria-label="Close notification panel"></button>
            </div>
